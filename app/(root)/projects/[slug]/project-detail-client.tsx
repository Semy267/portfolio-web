...
export default function ProjectDetailClient({
  ...
  const previewSecret = searchParams.get("preview");
  const isPreview = Boolean(previewSecret);
  const queryClient = useQueryClient();

  // Hydrate React Query cache with initialData from Server Component
  useEffect(() => {
    if (initialData && !isPreview) {
      queryClient.setQueryData(["public", "project", slug], {
        data: initialData,
      });
    }
  }, [initialData, slug, isPreview, queryClient]);

  const publicQuery = useGetProjectDetail(isPreview ? "" : slug);
  const previewQuery = useGetProjectPreview(
    isPreview ? slug : "",
    previewSecret || undefined,
  );

  const queryResult = isPreview ? previewQuery : publicQuery;
  const project = queryResult.project || initialData;
  const isLoading = !project && queryResult.isLoading;
  ...
  return (
    <div className="py-16">
      <div className="container max-w-4xl space-y-12">
        {/* Preview Banner */}
        {isPreview && (
          <div className="border-4 border-border bg-accent-yellow text-foreground p-4 shadow-hard flex flex-col sm:flex-row sm:items-center justify-between gap-3 font-mono font-bold text-sm">
            <div className="flex items-center gap-2">
              <Eye className="w-5 h-5 shrink-0" />
              <span>
                PREVIEW MODE — Status: [{project.status}] (Unpublished Draft /
                Revision)
              </span>
            </div>
            <span className="text-xs uppercase bg-foreground text-background px-2.5 py-1 w-fit border border-border">
              Admin Preview
            </span>
          </div>
        )}
